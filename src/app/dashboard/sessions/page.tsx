    ...
    },
    {
      ...
      render: (s: VoiceSession) => (
        <span className="text-dash-text-secondary truncate block max-w-xs">
          {s.project_brief || s.transcript?.slice(0, 80) || '—'}
        </span>
      ),
    },
    {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-dash-text-muted mt-1">
            Review customer voice agent conversations and convert to projects
          </p>
        </div>
        <button
          onClick={loadSessions}
          className="flex items-center gap-2 rounded-lg border border-dash-border bg-dash-surface px-3 py-2 text-sm text-dash-text-secondary hover:bg-dash-surface-2 transition cursor-pointer"
        >
      ...
      {!loading && sessions.length === 0 ? (
        <EmptyState
          ...
          description="When customers use the voice agent on the landing page, their sessions will appear here."
        />
      ) : (
        <DataTable
          columns={columns}
          data={sessions}
          onRowClick={(s) => router.push(`/dashboard/sessions/${s.id}`)}
        />
      )}
    </div>
  );
